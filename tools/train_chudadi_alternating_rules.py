...
def parse_args():
    ...
    parser.add_argument("--checkpoint", required=True, help="Existing V2 .pth checkpoint to continue from")
    ...
    parser.add_argument("--switch-games", type=int, default=10, help="Rule switch period in completed games")
...
def main():
    ...
    for game_index in range(1, args.total_games + 1):
        ...
        # Opponents are frozen snapshots so the learner does not chase its own
        # partially-updated policy inside the same game.
        ...
        if game_index % args.log_every_games == 0:
            averages = [round(total / args.log_every_games, 4) for total in payoff_sums]
            rule = "northern" if use_northern else "southern"
            print(
                f"games={game_index} rule={rule} replay={len(replay)} "
